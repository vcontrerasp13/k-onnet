...
async function fetchingData(currentPage: number, search: string, type: string, name: string) {

    console.log(name)
    console.log('Feching info desde server...');
    const data: OrdenResponse = await fetch(`${process.env.WIN_WIN_URL}?${type}=${search}&shippingAddress=${name}&paymentStatus=pagado&page=${currentPage}`, {
        method: 'GET',
        headers: {
...
async function TablePedidos({ currentPage, search, type, name }: Params) {

    const data: OrdenResponse = await fetchingData(currentPage, search, type,name)
                        ...
                        ordenes.map((orden: Orden) => {
                            ...
                            return (
                                <TableRow className=" text-xs" key={orden.cabecera_pedido[0].numero_orden}>
                                    ...
                                    <TableCell>
                                        <p className={`text-black text-center ${colorPago} p-2  rounded-lg`}>
